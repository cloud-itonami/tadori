(ns tadori.methods.social
  "social.cljc — 辿 — Trace (authorized on-chain transaction tracing + actor attribution) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from tadori's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "辿 — Trace (authorized on-chain transaction tracing + actor attribution) が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over tadori's own domain (Non-profit religious-corp authorized, case-anchored on-chain transaction-tracing + actor-attribution actor. kotoba-EAVT-native…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "tadori R0: live social posting is Council Lv6+ + operator + "
